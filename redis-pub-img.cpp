...
std::ifstream f("/workspace/val.json");
...
void producerThread(redisContext* redis) {
    ...
    const char* channel = "frame:eo";
    std::chrono::high_resolution_clock::time_point startTime, endTime;

    for(int i =0; i <data["images"].size() ; i++){
        std::cout << data["images"][i]["id"]<<" "<<data["images"][i]["file_name"] << std::endl;

        std::string image_path = data["images"][i]["file_name"];
        ...
        int idValue = data["images"][i]["id"].get<int>();
        redisReply* reply2 = (redisReply*)redisCommand(redis, "PUBLISH %s %d", "frame:eo:id",idValue);
